...
- `rtc-data-audio`: default low-latency WebRTC DataChannel PCM path.
...
- Audio codec v1 is PCM s16le with per-frame sample rate metadata.
- Radio RX publishers apply transport-edge direct integer decimation for high-rate PCM, so 48 kHz native soundcard frames are sent as 24 kHz without touching the native source or digital decode ring buffer.
...
| Contracts/protocol | done | Transport schemas only accept `rtc-data-audio` and `ws-compat`; generic PCM frame aliases remain compatible with WS exports. |
| Server transport | done | DataChannel manager, signaling route, source router, RX/TX bridge, and fallback offers are implemented. |
| Web transport | done | Browser DataChannel client uses shared playback/capture AudioWorklets and falls back to WS without recreating audio runtime. |
| Packaging/platform | done | Docker, Electron, and Linux server use embedded `node-datachannel` with degraded startup when unavailable. |
