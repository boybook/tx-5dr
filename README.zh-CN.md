...
| `tx5dr enable-livekit` | 安装并启用 LiveKit（可选，可后续补装） |
| `tx5dr disable-livekit` | 禁用 LiveKit，切换到 ws-compat 模式 |
...
- `tx5dr`：后端应用和 Web API
- `livekit-server` *（可选）*：随包提供的实时音频与 signaling 服务
- `nginx`：对外反向代理与 HTTPS 入口

`install.sh` 与 `tx5dr` 命令行会负责这些组件的安装、配置、诊断和联动重启。安装过程中可以选择是否安装 LiveKit。不装 LiveKit 时，语音功能通过 WebSocket 音频（ws-compat）传输 — 随时可通过 `sudo tx5dr enable-livekit` 补装。
...
- **Node.js 20+**（安装脚本自动安装）
- **nginx**（自动安装）
- **LiveKit** *（可选）*：安装包内置 `livekit-server`（`/usr/share/tx5dr/bin/livekit-server`）。不装 LiveKit 时语音使用 WebSocket 音频（ws-compat），所有功能均可正常使用
- 语音通联功能需要 **HTTPS**（在 `/etc/nginx/conf.d/tx5dr.conf` 中配置 SSL）
- 启用 LiveKit 后，浏览器通过当前站点的同源 `/livekit` 路径接入 signaling（`7880/tcp` 无需公网开放），但需额外放通 `7881/tcp` 和 `50000-50100/udp` 供媒体链路使用
- 如存在独立域名、额外反向代理或非标准路径，可在“系统设置 > 实时音频”中填写自定义实时语音入口（高级覆盖）；同一页面也统一提供 LAN / 公网自动 / 公网手动 三种 LiveKit 媒体网络模式
- Linux Server 与 Docker 都会根据这套配置生成托管的 `livekit.resolved.yaml` 运行文件；请勿直接手改生成后的 YAML
- 下载源覆盖开关：如需强制指定源，可在 `/etc/tx5dr/config.env` 中设置 `TX5DR_DOWNLOAD_SOURCE=github|oss|auto`
- 现在支持从“系统设置 > 性能诊断”发起仅针对 `server` 子进程的 CPU Profile；也可在 `/etc/tx5dr/config.env` 中设置 `TX5DR_SERVER_CPU_PROFILE=1` 做手动覆盖。不会影响 nginx、LiveKit、Electron 主进程或其他子进程
...
以上命令以独立模式启动，使用 WebSocket 音频传输（ws-compat，延迟约 50–100 ms）。如需更低延迟的 LiveKit/WebRTC 语音（延迟约 20–50 ms）：
...
- `yarn dev` / `yarn dev:electron` 会尽量自动托管本地 LiveKit 进程及其凭据文件，并通过同源 `/livekit` 反代给浏览器
