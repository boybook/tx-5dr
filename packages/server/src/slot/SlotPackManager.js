 ...
 * 负责去重、优化选择和维护最优解码结果
 */
export class SlotPackManager extends EventEmitter {
    constructor() {
        super();
        this.slotPacks = new Map();
    }
    /**
    ...
    processDecodeResult(result) {
        ...
        const correctedFrames = result.frames.map(frame => {
            ...
            const windowOffsetSec = (result.windowOffsetMs || 0) / 1000;
            const correctedDt = originalDt - windowOffsetSec;
            // 如果有窗口偏移，显示校正信息
            if (result.windowOffsetMs && result.windowOffsetMs !== 0) {
     ...
     * 创建新的 SlotPack
     */
    createSlotPack(slotId, timestamp) {
        // 从 slotId 中提取时隙开始时间
     ...
     * 按照添加顺序排列，而不是按信号强度排序
     */
    deduplicateAndOptimizeFrames(frames) {
        if (frames.length === 0)
            return [];
     ...
     * 清理过期的时隙包（超过指定时间的）
     */
    cleanupExpiredSlotPacks(maxAgeMs = 60000) {
        const now = Date.now();
        let cleanedCount = 0;
...
//# sourceMappingURL=SlotPackManager.js.map