...
import { resampleAudioProfessional, normalizeAudioVolume, analyzeAudioQualityDetailed } from '../utils/audioUtils.js';
 ...
 * 使用 wsjtx-lib 进行解码
 */
export class WSJTXDecodeWorkQueue extends EventEmitter {
    ...
    async push(request) {
        ...
        try {
            ...
            let resampledAudioData;
            if (request.sampleRate && request.sampleRate !== 12000) {
                console.log(`🔄 [解码队列] 重采样: ${request.sampleRate}Hz -> 12000Hz`);
                resampledAudioData = await resampleAudioProfessional(originalAudioData, request.sampleRate, 12000, 1, // 单声道
...
//# sourceMappingURL=WSJTXDecodeWorkQueue.js.map