...
export class TransmissionPipeline {
  ...
  private async handleMixedAudioReady(
    ...
    try {
      ...
      activeLeaseId = await this.deps.physicalTxCoordinator.acquireLease({
        ...
        validateStart: () => {
          this.deps.validateDigitalFrameStart?.(audioForTransmission.operatorIds, audioForTransmission.tracks);
  ...
  private async replaceCommittedFrameAudio(
    ...
    this.deps.validateDigitalFrameStart?.(participantOperatorIds, aligned.tracks);
