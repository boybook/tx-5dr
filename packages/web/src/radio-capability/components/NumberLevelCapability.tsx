...
export function NumberLevelCapability({ descriptor, capabilityId, state, interactive, scope, onWrite, draftEditor, showSliderInput = true, tooltipContent, showInlineLabel = true }: CapabilityComponentProps) {
  ...
  const detailsTooltip = { content: tooltipContent, isDisabled: !tooltipContent, delay: 350, closeDelay: 0, size: 'sm' as const, classNames: { content: 'max-w-[300px] text-xs' } };
  if (!descriptor.writable) return <Tooltip {...detailsTooltip}><span className="cap-item">{showInlineLabel && <span className="cap-label text-default-500">{label}</span>}<span className="cap-number-label">{actualText}</span></span></Tooltip>;
  ...
  return <Tooltip {...detailsTooltip} isDisabled={!showInput || !tooltipContent}><span className="cap-item">
    {showInlineLabel && (showInput ? labelElement : <Tooltip {...detailsTooltip}>{labelElement}</Tooltip>)}
    ...
    {showInput && <>
    <span className="cap-input-target" onClick={event => {
    ...
    <Input size="sm" type="number" aria-label={t(descriptor.labelI18nKey)} value={draftEditor?.text ?? edit.input} placeholder="—"
      ...
      onValueChange={draftEditor?.onChange ?? edit.edit} onBlur={draftEditor ? undefined : event => {
        // Keyboard navigation into the panel header leaves this edit without applying it.
        if (event.relatedTarget instanceof Element && event.relatedTarget.closest('[data-capability-navigation]')) edit.cancel();
        else edit.commit();
