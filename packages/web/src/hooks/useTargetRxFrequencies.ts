 ...
 * 提取其频率偏移（freq字段）
 *
 * @returns RX频率列表，每项包含操作员ID、呼号和频率
 */
export function buildTargetRxFrequencies(
  operators: OperatorStatus[],
  slotPacks: SlotPack[]
): RxFrequency[] {
  const result: RxFrequency[] = [];
  const recentSlotPacks = [...slotPacks].reverse();
...
export const useTargetRxFrequencies = (): RxFrequency[] => {
  ...
  return useMemo(
    () => buildTargetRxFrequencies(operators, slotPacksState.slotPacks),
