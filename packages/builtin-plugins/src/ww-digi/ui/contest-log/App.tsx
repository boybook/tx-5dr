...
const text = {
  zh: { reconcile: '重新对账', download: '下载 Cabrillo', empty: '暂无比赛通联', time: 'UTC', call: '对方呼号', exchange: '交换', mode: '模式', band: '波段', status: '状态', included: '计入', excluded: 'X-QSO', dupe: '重复' },
  ja: { reconcile: '照合', download: 'Cabrillo を保存', empty: 'QSO はありません', time: 'UTC', call: '相手局', exchange: '交換', mode: 'モード', band: 'バンド', status: '状態', included: '有効', excluded: 'X-QSO', dupe: '重複' },
  en: { reconcile: 'Reconcile', download: 'Download Cabrillo', empty: 'No contest QSOs', time: 'UTC', call: 'Callsign', exchange: 'Exchange', mode: 'Mode', band: 'Band', status: 'Status', included: 'Included', excluded: 'X-QSO', dupe: 'Dupe' },
};
    ...
    <main>
      <header>
        ...
        <div className="actions">
          <span className={`health ${state?.health.state || 'unknown'}`}>{state?.health.state || '...'}</span>
          <button type="button" disabled={busy} onClick={() => void reconcile()}>{t.reconcile}</button>
          <button
            type="button"
            className="primary"
          ...
          <tbody>
            {rows.map((record) => (
              <tr key={record.qsoId} className={record.status === 'x-qso' ? 'excluded' : ''}>
                ...
                <td><button type="button" className="status" disabled={busy} onClick={() => void toggle(record)}>{record.status === 'x-qso' ? t.excluded : record.dupe ? t.dupe : t.included}</button></td>
              </tr>
            ))}
